...
import { lastSyncTimestamp, isSyncing, incrementSyncTrigger } from '../composables/useSync';
...
export const SyncService = {
  ...
  async sync() {
    ...
    try {
      ...
      const changes = {
        ...
        locations: await db.locations.where('sync_status').equals('pending').toArray(),
        batches: await db.batches.where('sync_status').equals('pending').toArray(),
        count_sessions: await db.count_sessions.where('sync_status').equals('pending').toArray(),
        ledger_entries: await db.ledger_entries.where('sync_status').equals('pending').toArray(),
        vessels: await db.vessels.where('sync_status').equals('pending').toArray(),
        batch_additions: await db.batch_additions.where('sync_status').equals('pending').toArray(),
        batch_readings: await db.batch_readings.where('sync_status').equals('pending').toArray(),
        packaging_runs: await db.packaging_runs.where('sync_status').equals('pending').toArray(),
        categories: await db.categories.where('sync_status').equals('pending').toArray(),
        variance_events: await db.variance_events.where('sync_status').equals('pending').toArray(),
        par_levels: await db.par_levels.where('sync_status').equals('pending').toArray(),
        allocations: await db.allocations.where('sync_status').equals('pending').toArray(),
        recipes: await db.recipes.where('sync_status').equals('pending').toArray(),
        recipe_items: await db.recipe_items.where('sync_status').equals('pending').toArray(),
        milestone_templates: await db.milestone_templates.where('sync_status').equals('pending').toArray(),
        batch_milestones: await db.batch_milestones.where('sync_status').equals('pending').toArray(),
        batch_locations: await db.batch_locations.where('sync_status').equals('pending').toArray(),
        batch_volume_adjustments: await db.batch_volume_adjustments.where('sync_status').equals('pending').toArray(),
        batch_volume_snapshots: await db.batch_volume_snapshots.where('sync_status').equals('pending').toArray(),
        batch_location_transfers: await db.batch_location_transfers.where('sync_status').equals('pending').toArray()
      };
      ...
      const { updates, serverTimestamp, inventory_snapshot, orgStatus } = response.data;
  ...
  async markAsSynced(changes) {
    const mark = async (table, items) => {
      ...
      await db.table(table).where('id').anyOf(ids).modify({ sync_status: 'synced' });
    };
  ...
  async applyServerUpdates(updates, inventory_snapshot = null) {
    ...
    await apply(BatchVolumeAdjustmentRepository, updates.batch_volume_adjustments);
    await apply(BatchVolumeSnapshotRepository, updates.batch_volume_snapshots);
    await apply(BatchLocationTransferRepository, updates.batch_location_transfers);
    ...
    if (updates.ledger_entries) {
      for (const entry of updates.ledger_entries) {
        if (inventory_snapshot) {
          ...
          await LedgerRepository.applyRemoteEntry({ ...entry, sync_status: 'synced' });
        }
      }
    }
  }
