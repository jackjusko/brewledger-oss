...
export const BatchCostService = {
  async compute(batchId) {
    ...
    // Only water/liquid additions (ingredient additions create CONSUME and are counted from ledger)
    ...
    for (const entry of ledgerEntries) {
      if (entry.operation_type === 'packaging_supply') {
        ...
        const cost = entry.total_cost != null
          ? entry.total_cost
          : normalizeCost(entry.unit_cost, entry.quantity);
        ...
        const signedCost = absCost * sign;
        if (item?.category === 'Packaging') packaging += signedCost;
        else if (item?.category) materials += signedCost;
        else other += signedCost;
      }
    }
