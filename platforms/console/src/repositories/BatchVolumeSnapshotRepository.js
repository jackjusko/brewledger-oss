...
async function rebuildDerivedAdjustments(batchLocationId, orgId, userName) {
  const snapshots = await getSortedSnapshots(batchLocationId);
  const now = new Date().toISOString();

  // Remove prior derived adjustments so backdated snapshots recalc cleanly
  ...
  // Create deltas between sequential snapshots (baseline snapshot creates no delta)
  ...
  // Sync current_volume to latest measured value
...
export const BatchVolumeSnapshotRepository = {
  ...
  async recordSnapshot({ batch_location_id, measured_volume, measured_at, method, note }) {
    ...
    await rebuildDerivedAdjustments(batch_location_id, orgId, userName);
    return snapshot;
  },
