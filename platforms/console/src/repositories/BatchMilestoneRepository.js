...
export const BatchMilestoneRepository = {
  async getByBatchId(batchId) {
    const rows = await db.batch_milestones.where('batch_id').equals(batchId).toArray();
  ...
  async create(milestone) {
    ...
    const newMilestone = { ...milestone, id: uuidv4(), org_id: orgId, completed: milestone.completed !== false, occurred_at: milestone.occurred_at || now, created_at: now, updated_at: now, sync_status: 'pending', version: 1 };
  ...
  async update(id, updates) {
    ...
    const updated = { ...updates, updated_at: now, sync_status: 'pending', version: (current.version || 0) + 1 };
  ...
  async ensure(batchId, milestoneDefinitionId, occurredAt = null) {
    const existing = await db.batch_milestones.where('batch_id').equals(batchId).filter(m => m.milestone_definition_id === milestoneDefinitionId).first();
    if (existing) {
      if (!existing.completed) return await this.update(existing.id, { completed: true, occurred_at: occurredAt || new Date().toISOString() });
      return existing;
    }
    return await this.create({ batch_id: batchId, milestone_definition_id: milestoneDefinitionId, occurred_at: occurredAt || new Date().toISOString(), completed: true });
  },
  async initForBatch(batchId, definitions, initialStatus, knockedOutAt = null) {
    ...
    if (initialStatus === 'BREWED' && firstDef) {
      await this.create({ batch_id: batchId, milestone_definition_id: firstDef.id, occurred_at: now, completed: true });
    }
  },
  ...
  async applyRemoteUpsert(milestone) { await db.batch_milestones.put(milestone); }
};
