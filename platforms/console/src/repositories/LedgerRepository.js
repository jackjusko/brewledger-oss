...
export const LedgerRepository = {
  async addEntry(entry) {
    ...
    const newEntry = {
      ...
      qbo_customer_id: entry.qbo_customer_id || null,
      qbo_invoice_id: entry.qbo_invoice_id || null,
    ...
    // When a Finished Beer item is affected, soft-delete any beer items that are now at zero on-hand
  ...
  /** Returns on-hand quantity per location for a given item. Used for context-aware dropdowns (e.g. Sales Order). */
  async getOnhandByItem(itemId) {
    const rows = await db.onhand_cache.where('item_id').equals(itemId).toArray();
    return rows.map(r => ({ location_id: r.location_id, quantity: r.quantity }));
  },
