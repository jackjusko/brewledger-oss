...
export const BatchReadingRepository = {
  ...
  async getByBatchLocationId(batchLocationId) {
    return await db.batch_readings.where('batch_location_id').equals(batchLocationId).reverse().sortBy('measured_at');
  },
  ...
  async add(reading) {
    ...
    const newReading = {
      ...
      id: uuidv4(),
      org_id: orgId,
      batch_location_id: reading.batch_location_id ?? null,
