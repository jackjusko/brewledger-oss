...
export const BatchRepository = {
  async getAll() {
    ...
    if (orgId) {
      return await db.batches.where('org_id').equals(orgId).reverse().sortBy('batch_date');
    }
  ...
  async create(batch) {
    ...
    if (!milestoneDefinitions || milestoneDefinitions.length === 0) {
      let templateId = milestoneTemplateId || (await this.getDefaultTemplateId());
      let template = templateId ? await MilestoneTemplateRepository.getById(templateId) : null;
      if (!template) {
      ...
      if (template) {
        ...
        milestoneDefinitions = MilestoneTemplateRepository.DEFAULT_MILESTONES.map(m => ({
          ...m,
          id: uuidv4()
        }));
      }
    ...
    try {
      ...
      await BatchMilestoneRepository.initForBatch(
        newBatch.id,
