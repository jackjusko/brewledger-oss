...
// Forced last milestone (TTB); system-appended, not user-removable
...
// Default milestone content (labels and descriptions only; no statusType)
...
// Legacy type -> default template index for migration
...
export const MilestoneTemplateRepository = {
  DEFAULT_MILESTONES,
  PRODUCTION_COMPLETE_LABEL,
  FORCED_LAST_MILESTONE,

  async getAll() {
    const { orgId } = await getContext();
    if (!orgId) return [];
    return await db.milestone_templates
      .where('org_id')
      .equals(orgId)
      .sortBy('updated_at')
  ...
  async delete(id) {
    await db.milestone_templates.delete(id);
  },
  ...
  async applyRemoteUpsert(template) {
    await db.milestone_templates.put(template);
  }
};
