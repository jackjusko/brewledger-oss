<template>
  <div class="p-3 pb-20">
    <div class="mb-3">
      <h1 class="text-xl font-bold text-gray-900 dark:text-gray-50">Serving</h1>
      <p class="text-xs text-gray-500 dark:text-gray-400 mt-0.5">
        All serving locations (vessels and standalone). Volume from ledger. Set volume when you measure, record removal when beer is poured or lost.
      </p>
    </div>

    <div v-if="loading" class="text-sm text-gray-500 dark:text-gray-400 py-4">Loading tanks…</div>
    <div v-else-if="tanks.length === 0" class="space-y-3 text-sm text-gray-600 dark:text-gray-400">
      <p v-if="unlinkedServingCount > 0" class="rounded-lg border border-amber-200 dark:border-amber-800 bg-amber-50 dark:bg-amber-900/20 p-3">
        You have <strong>{{ unlinkedServingCount }}</strong> Serving Tank(s) that aren’t linked to a location. Edit them on
        <router-link to="/vessels" class="text-blue-600 dark:text-blue-400 underline font-medium">Vessels</router-link>
        and select a <strong>location</strong> so they appear here.
      </p>
      <p v-else class="bg-white dark:bg-gray-800 p-4 rounded-lg border border-gray-200 dark:border-gray-700">
        No serving tanks yet. On <router-link to="/vessels" class="text-blue-600 dark:text-blue-400 underline font-medium">Vessels</router-link>: create or edit a vessel, set type to <strong>Serving Tank</strong>, and select a <strong>location</strong>. Linked tanks will show here.
      </p>
    </div>
    <div v-else class="grid gap-3">
      ...
      >
        <div class="flex items-start justify-between gap-2">
          <div>
            <div class="font-semibold text-gray-900 dark:text-gray-100">{{ tank.vesselName }}</div>
            <div class="text-xs text-gray-500 dark:text-gray-400 mt-0.5">
              {{ tank.locationName }}{{ tank.vesselId ? '' : ' (no vessel)' }}
            </div>
          </div>
          <div class="font-mono text-gray-900 dark:text-gray-100 text-sm">
        ...
        <div class="mt-3 flex flex-wrap gap-2">
          <button v-if="tank.vesselId" type="button" class="px-3 py-1.5 bg-gray-200 dark:bg-gray-700 text-gray-800 dark:text-gray-200 rounded-md text-sm font-medium disabled:opacity-50" :disabled="tank.conflict || !tank.beerItem" @click="openSetVolume(tank)">
            Set volume
          </button>
          <button type="button" class="px-3 py-1.5 bg-blue-600 hover:bg-blue-700 text-white rounded-md text-sm font-medium disabled:opacity-50" :disabled="tank.vesselId && (tank.conflict || !tank.beerItem)" @click="openRecordRemoval(tank)">
            Record removal
          </button>
          <button
            v-if="tank.vesselId && tank.beerItem && (tank.onHand == null ? 0 : tank.onHand) > 0"
            type="button"
            class="px-3 py-1.5 border border-amber-400 text-amber-700 dark:text-amber-300 rounded-md text-sm font-medium disabled:opacity-50"
    ...
    <div v-if="modals.setVolume" class="fixed inset-0 z-50 bg-black/50 flex items-center justify-center p-4">
      <div class="bg-white dark:bg-gray-800 rounded-lg shadow-xl w-full max-w-md p-5">
        <h3 class="text-lg font-semibold text-gray-900 dark:text-gray-100 mb-3">Set volume — {{ setVolumeTank?.vesselName }}</h3>
        <p class="text-sm text-gray-600 dark:text-gray-400 mb-3">
          This will reconcile ledger at <strong>{{ setVolumeTank?.locationName }}</strong>. Current on-hand: <strong>{{ setVolumeTank?.onHand != null ? setVolumeTank.onHand.toFixed(2) : '0' }}</strong> bbl.
        </p>
        <div class="mb-4">
          <label class="block text-sm font-medium mb-1 text-gray-700 dark:text-gray-300">Measured volume (bbl)</label>
          <input v-model.number="forms.setVolume.measured_volume" type="number" step="0.01" min="0" class="w-full border border-gray-300 dark:border-gray-600 p-2 rounded-md dark:bg-gray-700 dark:text-white" placeholder="0.00">
        </div>
        <div class="flex justify-end gap-2">
          <button type="button" class="px-3 py-1.5 text-gray-700 dark:text-gray-300 border rounded-md" @click="modals.setVolume = false">Cancel</button>
          <button type="button" class="px-3 py-1.5 bg-blue-600 text-white rounded-md" @click="saveSetVolume" :disabled="saving || forms.setVolume.measured_volume == null">Save</button>
        </div>
      </div>
    </div>
    ...
    <div v-if="modals.recordRemoval" class="fixed inset-0 z-50 bg-black/50 flex items-center justify-center p-4">
      <div class="bg-white dark:bg-gray-800 rounded-lg shadow-xl w-full max-w-md p-5">
        <h3 class="text-lg font-semibold text-gray-900 dark:text-gray-100 mb-3">Record removal — {{ recordRemovalTank?.vesselName }}</h3>
        <p class="text-sm text-gray-600 dark:text-gray-400 mb-3">
          Record a pour or loss from <strong>{{ recordRemovalTank?.locationName }}</strong>. This posts a CONSUME for TTB Line 21.
        </p>
        <div class="mb-4">
          <label class="block text-sm font-medium mb-1 text-gray-700 dark:text-gray-300">Amount to remove (bbl)</label>
          <input v-model.number="forms.recordRemoval.quantity" type="number" step="0.01" min="0" class="w-full border border-gray-300 dark:border-gray-600 p-2 rounded-md dark:bg-gray-700 dark:text-white" placeholder="0.00">
        ...
        <div class="mb-4">
          ...
          <input v-model="forms.recordRemoval.note" type="text" class="w-full border border-gray-300 dark:border-gray-600 p-2 rounded-md dark:bg-gray-700 dark:text-white" placeholder="e.g. Tap 1 pour">
        </div>
        <div class="flex justify-end gap-2">
          <button type="button" class="px-3 py-1.5 text-gray-700 dark:text-gray-300 border rounded-md" @click="modals.recordRemoval = false">Cancel</button>
          <button type="button" class="px-3 py-1.5 bg-blue-600 text-white rounded-md" @click="saveRecordRemoval" :disabled="saving || !forms.recordRemoval.quantity || forms.recordRemoval.quantity <= 0 || recordRemovalExceedsOnHand">Record</button>
        </div>
      </div>
    </div>
    ...
    <div v-if="modals.setEmpty" class="fixed inset-0 z-50 bg-black/50 flex items-center justify-center p-4">
      <div class="bg-white dark:bg-gray-800 rounded-lg shadow-xl w-full max-w-md p-5">
        <h3 class="text-lg font-semibold text-gray-900 dark:text-gray-100 mb-3">Set empty — {{ setEmptyTank?.vesselName }}</h3>
        ...
        <div class="flex justify-end gap-2">
          <button type="button" class="px-3 py-1.5 text-gray-700 dark:text-gray-300 border rounded-md" @click="modals.setEmpty = false">Cancel</button>
          <button type="button" class="px-3 py-1.5 bg-blue-600 text-white rounded-md" @click="saveSetEmpty" :disabled="saving">Set empty</button>
        </div>
      </div>
    </div>
    ...
    <div v-if="modals.recordRemovalLocation" class="fixed inset-0 z-50 bg-black/50 flex items-center justify-center p-4">
      <div class="bg-white dark:bg-gray-800 rounded-lg shadow-xl w-full max-w-md p-5">
        <h3 class="text-lg font-semibold text-gray-900 dark:text-gray-100 mb-3">Record removal — {{ recordRemovalLocationTank?.locationName }}</h3>
        <p class="text-sm text-gray-600 dark:text-gray-400 mb-3">
          Record a pour or loss from this location. Choose the beer item and amount. This posts a CONSUME for TTB Line 21.
        </p>
        <div v-if="recordRemovalLocationItems.length === 0" class="mb-4 text-sm text-gray-500 dark:text-gray-400">
        ...
        <template v-else>
          <div class="mb-4">
            ...
            <select v-model="forms.recordRemovalLocation.item_id" class="w-full border border-gray-300 dark:border-gray-600 p-2 rounded-md dark:bg-gray-700 dark:text-white">
          ...
          <div class="mb-4">
            <label class="block text-sm font-medium mb-1 text-gray-700 dark:text-gray-300">Amount to remove (bbl)</label>
            <input v-model.number="forms.recordRemovalLocation.quantity" type="number" step="0.01" min="0" class="w-full border border-gray-300 dark:border-gray-600 p-2 rounded-md dark:bg-gray-700 dark:text-white" placeholder="0.00">
          ...
          <div class="mb-4">
            <label class="block text-sm font-medium mb-1 text-gray-700 dark:text-gray-300">Note (optional)</label>
            <input v-model="forms.recordRemovalLocation.note" type="text" class="w-full border border-gray-300 dark:border-gray-600 p-2 rounded-md dark:bg-gray-700 dark:text-white" placeholder="e.g. Tap 1 pour">
          </div>
        </template>
        <div class="flex justify-end gap-2">
          <button type="button" class="px-3 py-1.5 text-gray-700 dark:text-gray-300 border rounded-md" @click="modals.recordRemovalLocation = false">Cancel</button>
          <button type="button" class="px-3 py-1.5 bg-blue-600 text-white rounded-md" @click="saveRecordRemovalLocation" :disabled="saving || recordRemovalLocationItems.length === 0 || !forms.recordRemovalLocation.item_id || !forms.recordRemovalLocation.quantity || forms.recordRemovalLocation.quantity <= 0 || recordRemovalLocationExceedsOnHand">Record</button>
        </div>
      </div>
    </div>
  </div>
...
async function loadTanks() {
  ...
  try {
    ...
    unlinkedServingCount.value = vessels.filter(
      (v) => !v.deleted_at && isServingType(v) && !hasLocationId(v)
    ).length;
...
function openSetVolume(tank) {
  if (tank.conflict || !tank.beerItem) {
    alert(tank.conflict ? 'This location has more than one beer. Resolve inventory so only one beer has quantity here.' : 'Add beer to this location first (e.g. via Mark Production Complete).');
    return;
  }
...
async function saveSetVolume() {
  ...
  if (!beerItem) {
    alert('No beer item at this location. Add beer first (e.g. via Mark Production Complete).');
    return;
  }
  saving.value = true;
  try {
    ...
    const onHand =
      setVolumeTank.value.onHand != null
        ? setVolumeTank.value.onHand
        : await LedgerRepository.getOnhand(beerItem.id, setVolumeTank.value.locationId);
    ...
    alert(e.message || 'Failed to set volume.');
  } finally {
    saving.value = false;
  }
}
...
async function saveRecordRemoval() {
  if (
    !recordRemovalTank.value ||
    !forms.value.recordRemoval.quantity ||
    forms.value.recordRemoval.quantity <= 0 ||
    recordRemovalExceedsOnHand.value
  )
    return;
  ...
  try {
    ...
    alert(e.message || 'Failed to record removal.');
  } finally {
    saving.value = false;
  }
}
...
async function saveRecordRemovalLocation() {
  ...
  if (
    !tank ||
    !forms.value.recordRemovalLocation.item_id ||
    !forms.value.recordRemovalLocation.quantity ||
    forms.value.recordRemovalLocation.quantity <= 0 ||
    recordRemovalLocationExceedsOnHand.value
  )
    return;
  saving.value = true;
  try {
...
async function saveSetEmpty() {
  ...
  if (!beerItem) {
    alert('No beer item at this location.');
    return;
  }
  saving.value = true;
  try {
    ...
    alert(e.message || 'Failed to set empty.');
  } finally {
    saving.value = false;
  }
}
